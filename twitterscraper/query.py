...
RELOAD_URL_USER = (
    "https://twitter.com/i/profiles/show/{u}/timeline/tweets?"
...
def query_tweets_once_generator(query, limit=None, lang="", pos=None):
    ...
    try:
        ...
        logger.info(
            "Program interrupted by user. Returning tweets gathered " "so far..."
        )
    ...
    try:
        ...
        try:
            for new_tweets in pool.imap_unordered(
                partial(query_tweets_once, limit=limit_per_pool, lang=lang), queries
            ):
                all_tweets.extend(new_tweets)
                logger.info(
                    "Got {} tweets ({} new).".format(len(all_tweets), len(new_tweets))
                )
        except KeyboardInterrupt:
            logger.info(
